...
export const AdminBookUpload = () => {
  ...
  const validateFile = (file, type, maxSize) => {
    ...
    if (!validTypes[type].includes(file.type)) {
      return `Invalid file type. Please upload a ${type === "pdf" ? "PDF" : "JPG/PNG"} file.`;
    }
  ...
  const handleFileChange = (e, type) => {
    ...
    if (validationError) {
      ...
      setFormData((prev) => ({ ...prev, [`${type}File`]: null, [`${type}Preview`]: null }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const responseData = await postData("http://localhost:3000/add-book", uploadData);
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-lg max-w-full">
      <h2 className="text-2xl font-bold text-primary mb-4">Upload New Book</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Title */}
        <div>
          <label className="block text-secondary font-semibold mb-2">Book Title</label>
          <input
            type="text"
            name="title"
            value={title}
        ...
        {/* PDF Upload */}
        <div>
          <label className="block text-secondary font-semibold mb-2">Upload PDF (Max 50MB)</label>
          <input
            type="file"
        ...
        {/* Image Upload */}
        <div>
          <label className="block text-secondary font-semibold mb-2">Upload Cover Image (JPG/PNG, Max 5MB)</label>
          <input
            type="file"
        ...
        {(imagePreview || pdfPreview) && (
          <div className="border rounded-md p-3 bg-gray-50">
            ...
            {pdfPreview && (
              <a href={pdfPreview} target="_blank" rel="noreferrer" className="text-primary underline">
                View PDF
              </a>
            )}
          </div>
